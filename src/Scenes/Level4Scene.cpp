...
void Level4Scene::Initialize(entt::registry &registry)
{
    ...
    std::vector<std::tuple<AnchorType, Position, std::optional<std::pair<Vector2, Vector2>>>> anchors = {
        std::make_tuple(AnchorType::Static, Position{250.0f, 150.0f}, std::nullopt),
        std::make_tuple(AnchorType::Static, Position{475.0f, 200.0f}, std::nullopt),
        std::make_tuple(AnchorType::Dynamic, Position{475.0f, 600.0f}, std::make_pair(Vector2{475.0f, 600.0f}, Vector2{900.0f, 150.0f})),
        std::make_tuple(AnchorType::Static, Position{1220.0f, 250.0f}, std::nullopt),
        std::make_tuple(AnchorType::Static, Position{1450.0f, 400.0f}, std::nullopt),
    };
    ...
    std::vector<std::tuple<Obstacle, Position>> obstacles = {
        std::make_tuple(Obstacle{Vector2{10.0f, 200.0f}, true, 2.0f, 120.0f}, Position{1200.0f, 600.0f}),
    };
    ...
    registry.emplace<Position>(finishLine, 1650.0f, 600.0f);
    this->m_sceneEntities.emplace_back(finishLine);
}

void Level4Scene::Shutdown(entt::registry &registry)
{
